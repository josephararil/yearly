  ...
  function ProjectionChart({ stats }) {
    ...
    return (
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: "block", overflow: "visible" }}>
        ...
        {grid.map((v, i) => (
          <g key={i}>
            <line x1={x0} y1={sy(v)} x2={x1} y2={sy(v)} stroke="var(--hairline)" strokeWidth="1" />
            <text x={x0 - 6} y={sy(v) + 3} textAnchor="end" fontSize="9" fill="var(--text-3)" fontFamily="var(--font-mono)">{eurK(v)}</text>
          </g>
        ))}
        {showMonths.map((m) => (
          <text key={m} x={sx(MONTH_STARTS[m])} y={H - 8} textAnchor="middle" fontSize="9" fill="var(--text-3)">{MONTHS[m]}</text>
        ))}
        {/* target reference */}
        <line x1={x0} y1={sy(stats.target)} x2={x1} y2={sy(stats.target)} stroke="var(--text-2)" strokeWidth="1" strokeDasharray="3 3" />
        ...
        {!stats.complete && (
          <>
            <line x1={sx(stats.doy)} y1={sy(stats.spent)} x2={sx(365)} y2={sy(stats.projection)} stroke="var(--watch)" strokeWidth="2" strokeDasharray="5 4" strokeLinecap="round" />
            <circle cx={sx(365)} cy={sy(stats.projection)} r="3.2" fill="var(--watch)" />
            <circle cx={sx(stats.doy)} cy={sy(stats.spent)} r="3.6" fill={col} stroke="var(--surface)" strokeWidth="1.5" />
          </>
        )}
      </svg>
    );
        ...
        {months.map((m, i) => {
          ...
          return (
            <g key={m}>
              ...
              <text x={x + bw / 2} y={H - 4} textAnchor="middle" fontSize="9" fill="var(--text-3)">{MONTHS[m][0]}</text>
            </g>
          );
        })}
      </svg>
            ...
            return (
              <div key={c.id}>
                <button className="catbar-row" onClick={() => setSel(open ? null : c.id)} style={{ borderBottom: open ? "0" : undefined }}>
                  <CatIcon catId={c.id} size={34} radius={10} iconSize={18} />
                  <span className="catbar-main">
                    ...
                    <span className="catbar-sub">
                      ...
                      {mv != null && Math.abs(mv) > 0.05 && (
                        <span style={{ color: mv > 0 ? "var(--watch)" : "var(--good)" }}>{signedPct(mv)} MoM</span>
                      )}
                    </span>
                  </span>
                  <window.Icon name={open ? "chevronUp" : "chevronDown"} size={16} style={{ color: "var(--text-3)" }} />
                </button>
                {open && (
                  <div style={{ padding: "4px 2px 14px", borderBottom: "1px solid var(--hairline)" }}>
                    <CatTrend cid={c.id} catMonth={stats.catMonth} upToMonth={curMonth} />
                    <div className="muted" style={{ fontSize: 11.5, margin: "8px 2px 4px", textTransform: "uppercase", letterSpacing: "0.06em" }}>Recent in {cat.label}</div>
  ...
  function ActivityTab({ stats, onEditTx }) {
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div className="muted" style={{ textAlign: "center", fontSize: 12 }}>{list.length} of {stats.txns.length} entries</div>
      </div>
    );
  }
