...
- The pull window is a **stateless rolling 30 days** (`BUFFER_DAYS` in `scripts/bookmarklet.js`,
  same value and rationale as `sync.py`'s `BUFFER_DAYS` — catch late-settling/PENDING
  transactions), anchored to "now" instead of "last sync" since there's no local
  `.sync_state.json` on a phone. This keeps day-to-day runs fast (a handful of transactions, not
  the whole year). The app's cleaner still filters to the current year and the ingest endpoint's
  upsert is idempotent and field-preserving, so occasional over-fetching (e.g. after a long gap
  between runs) is harmless — just re-run it, or widen `BUFFER_DAYS` and regenerate the minified
  URL if you skip more than 30 days between imports.
