...
Four tables, applied via `npx wrangler d1 migrations apply yearly-db --remote`. Ten migration
...
`0008_amortize.sql`, `0009_tx_ts.sql`, `0010_fun_allocations.sql`.
...
-- fun_allocations TEXT  (nullable; JSON-serialised array of {person, amount} that refines the
--                        fun-budget overlay for partial/split transactions — see README "Partial
--                        and split fun allocation". User-owned, like fun/person.)
...
`NULL` when the tx isn't amortized). `fun_allocations` is a nullable `TEXT` holding a JSON-serialised
`{person, amount}[]` array, or `NULL` when the tx isn't split/partial. `updated_at` is a
**server-stamped ms epoch** on every write.
